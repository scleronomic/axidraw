...
def title():
    d = axidraw.Drawing(axidraw.text('Topography of the Western United States', axidraw.FUTURAM))
...
def main():
    ...
    for j in range(0, ROWS, 1):
        ...
        for q in range(0, 101, 25):
            ...
            for path in remove_flats(path):
                ...
                path = zip(x, y)
                path = axidraw.simplify_paths([path], 0.005)[0]
                paths.append(path)
    ...
    im = d.render(scale=109 * 1, line_width=0.3/25.4)  # show_axi_bounds=False, use_axi_bounds=False)
